...
#------- local_disp -------------
idx=grep("^#local_disp",lines)
if (length(idx)==1) {
	local_disp=gsub("^#local_disp\t","",lines[idx])
	if (local_disp=="TRUE") {
		local_disp=TRUE
	} else {
		local_disp=FALSE
...
cat("\t\tlocal_disp=",local_disp,"\n",file=logFile,append=T,sep="")
#------- group.disp -------------
idx=grep("^#group_disp",lines)
if (length(idx)==1) {
	group_disp=gsub("^#group_disp\t","",lines[idx])
	
	if (group_disp!="0" & group_disp!="1" & group_disp!="both") {
		cat("ERROR group_disp should either 'both', '0' or '1'\n",file=logFile,append=T,sep="")
		cat("STATUS KO\n",file=logFile,append=T,sep="")
		q(save="no",status=1)
	}
	if (group_disp=="0" | group_disp=="1") {
		group_disp=as.numeric(group_disp)
	}
} else {
	group_disp="both"
}
cat("\t\tgroup_disp=",group_disp,"\n",file=logFile,append=T,sep="")
#------- destranded -------------
...
cat("\t",date(),"\tRead data ...\n",file=logFile,append=T,sep="")
meth <- methylSigReadData(fileList, sample.ids = sample.id, assembly = "BosTau6",
			  treatment = treatment, context = "CpG", destranded=destranded,
			  minCount=min_coverage, maxCount=max_coverage
)

cat("\t",date(),"\tLaunch methylSig ...\n",file=logFile,append=T,sep="")
diffSig = methylSigCalc(
		meth,
		groups=c(1,0), dispersion=group_disp,
		local.disp=local_disp,local.meth=local_disp,
...
# Result analysis
###################################
pvalues=as.numeric(as.character(diffSig[,stat_value]))
mdiff=as.numeric(as.character(diffSig[,"meth.diff"]))
positions=paste(diffSig[,"chr"],".",diffSig[,"start"],sep="")

nbCpGTested=length(pvalues)
cat("INFO number of CpGs tested by methylSig=",nbCpGTested,"\n",file=logFile,append=T,sep="")

selectSignificative <- function (stat.threshold,methdiff_threshold) {
	selDMC=which(pvalues<stat.threshold & abs(mdiff)>=methdiff_threshold)

	if (sum(selDMC!=0)) {
		chr=as.character(diffSig[selDMC,"chr"])
		start=as.numeric(as.character(diffSig[selDMC,"start"]))
		pv=as.numeric(as.character(diffSig[selDMC,stat_value]))
		methDiff=as.numeric(as.character(diffSig[selDMC,"meth.diff"]))
		
		coordsDMC=paste(chr,".",start,sep="")
		coordsMeth=paste(as.character(meth[,"chr"]),".",as.numeric(as.character(meth[,"start"])),sep="")
		...
		for (i in 1:length(sample.id)) {
			selectedSummary=cbind(selectedSummary,
					      meth[selMeth,paste("coverage",i,sep="")],
					      meth[selMeth,paste("numCs",i,sep="")]
			)
		}
...
if (!is.null(selResults)) {#There are some results
	...
	rawp=as.numeric(as.character(diffSig[,"pvalue"]))
	...
	mdOthers=mdiff[!(positions %in% CpGSelected)]
	...
	output.file=paste(output_dir,"/MethylSig - ",title," - ",stat_value,stat_threshold2,sep="")
	write.table(file=paste(output.file,".txt",sep=""),selResults,sep="\t",row.names=F,quote=F)
} else {
	nbSignif=0
...
cat("RESULT number of DMCs detected by methylSig=",nbSignif,"\n",file=logFile,append=T,sep="")
cat("OUT ",output.file,".txt\n",file=logFile,append=T,sep="")
cat("STATUS OK\n",file=logFile,append=T,sep="")

#save(list=c("meth","diffSig"),file="methylSig_test1.RData")
